...
def calculate_carrier_counts_ids(mt, qc_done=True):
    """
    """"""
...
def get_lof_carriers(mt):
    ...
    # Annotate a row annotation for the number of hom alt and het carriers if the variant is LOF
    ...
    if (lof_homalt_count > 0) or (lof_het_count > 0):
        ...
        lofmt = lofmt.annotate_rows(hom_alt_carriers=hl.agg.filter(lofmt.GT.is_hom_var(), hl.agg.collect(mt.s)))
        ...
        carriers_ht.export("het_carriers.txt")
    else:
        carriers_ht = None

    return lof_homalt_count, lof_het_count, carriers_ht
...
def get_gene_damaging_carriers(mt, gene, report_all_homozygotes=True):
    ...
    lof_carriers = get_lof_carriers(genemt)
