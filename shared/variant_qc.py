...
def downsample_variants(mt, target_count, checkpoint_name, r2=0.2, bp_window_size=500000, ld_prune=False):
    ...
    if var_count > target_count:
        if ld_prune:
            ...
            ld_prune_count = mt.count_rows()
            if ld_prune_count > target_count:
                logging.info(f"Matrix table still has {ld_prune_count} rows after LD pruning. Downsampling further.")
...
def find_failing_vars(mt, checkpoint_name, prefix="", pheno_col=None, count_failing=False, snp_qd=2, indel_qd=3,
                      ab_allowed_dev_het=0.8, p_hwe=1e-6, sex_aware=False, min_call_rate=0.8, min_dp=10, min_gq=20):
    #######################################################
    ...
    if count_failing:
        ...
        # Mean DP
        failing_dp = mt.aggregate_rows(hl.agg.count_where(mt[failing_name].contains("failing_mean_DP")))
        failing_dp_perc = round(failing_dp / total_variants * 100, 2)
        filter_dict["failing_mean_dp"] = failing_dp

        # Mean GQ
        failing_gq = mt.aggregate_rows(hl.agg.count_where(mt[failing_name].contains("failing_mean_GQ")))
        failing_gq_perc = round(failing_gq / total_variants * 100, 2)
        filter_dict["failing_mean_gq"] = failing_gq
        ...
        logging.info(f"\nVariants failing VQSR filters: {failing_vqsr}, {failing_vqsr_perc}%"
                     ...
                     f"\nmean DP < {min_dp}: {failing_dp}, {failing_dp_perc}%"
                     f"\nmean GQ < {min_gq}: {failing_gq}, {failing_gq_perc}%"
...
def variant_quality_control(
    ...
    if (not utils.check_exists(checkpoint_name + "_failing_vars_annotated.mt/")) or force:
        mt_varannot = find_failing_vars(mt_abannot, checkpoint_name, prefix=annotation_prefix, pheno_col=pheno_col,
                                        ...
                                        min_call_rate=call_rate, min_dp=min_dp, min_gq=min_gq)
    else:
...
def find_variants_failing_by_pheno(mt, ab_allowed_dev_het=0.8, pheno_call_rate=0.9):
    """
    ...
    case_cond = hl.cond(hl.is_defined(mt.final_case_frac_het_gts_in_ab) &
                        (mt.final_case_frac_het_gts_in_ab < ab_allowed_dev_het),
    ...
    control_cond = hl.cond(hl.is_defined(mt.final_control_frac_het_gts_in_ab) &
                           (mt.final_control_frac_het_gts_in_ab < ab_allowed_dev_het),
    ...
    case_cr_cond = hl.cond(hl.is_defined(mt.sexaware_case_call_rate) &
                           (mt.sexaware_case_call_rate < pheno_call_rate),
    ...
    cont_cr_cond = hl.cond(hl.is_defined(mt.sexaware_cont_call_rate) &
                           (mt.sexaware_cont_call_rate < pheno_call_rate),
    ...
    mt = mt.annotate_globals(case_control_callrate_threshold=pheno_call_rate)
